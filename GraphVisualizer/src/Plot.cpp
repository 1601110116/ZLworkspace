...
void Plot::trackData1D(double* data, int len){

	graph.graphics.clear();

	unsigned int sx=Max(0,SX);
	unsigned int ex=Min(len,EX);
...
void Plot::trackData1D2(double* data, int len){
	...
	graph.graphics.lineStyle(0,0xff0099,1);
	graph.graphics.moveTo(0,Height-yscale*(data[sx]-SY));
...
//}
//private function drawFunction(fn:Function,name:String,lstyle:LineStyle):void{
//
//	var steplength:Number=(EX-SX)/Width;
//	var steps:int=(EX-SX)/steplength;
//
//	var fnres:Number;
//
//	var prevIsNaN:Boolean=true;
//
//	graphics.lineStyle(0,lstyle.color);
//
//	for (var i:Number = 0; i < steps+1; i++) {
//		fnres=fn(i*steplength+SX);
//		if(isNaN(fnres)){
